...
export default function CreateAccountBox() {
  ...
  const confirmBtnHandler = async (): Promise<void> => {
    const response = await checkAccountExists(inputPhone);
    ...
    if (response.exists === false) {
      ...
      setError("This phone already register");
    }
  };
  ...
  const finalConfirmBtnHandler = async (): Promise<void> => {
    if (isAccountConfirm) {
      setIdLoading(true);
      const response = await createAccount(inputUsername, inputPhone);
      setAccountDetails([
  ...
  return (
    <div className="p-4 rounded-lg bg-gray-300 mt-10">
      {isLoading && (
        ...
        <div>
          ...
          <div className="flex gap-4 mb-5">
            ...
            <div className="flex gap-2 ml-1">
              <button
                className="bg-gray-700 hover:bg-gray-800 text-white p-1 px-4 rounded-md"
          ...
          <div className="flex gap-4">
            ...
            <input
              type="text"
              value={inputUsername}
              onChange={(e) => setInputUsername(e.target.value as any)}
              placeholder="Input the Username"
              className="border-slate-500 rounded-lg pl-2 ml-12"
            />
            <button
              className={
                isAccountConfirm
          ...
          <div className="text-red-500 font-semibold text-lg mt-5">{error}</div>
